...
Desligamentos e ações de ciclo de vida já usam jobs persistentes no SQLite. Backups, uploads, restores e updates reutilizarão a mesma infraestrutura nas etapas correspondentes. Em produção, o worker é um processo independente da aplicação web.
...
O worker executa as operações suportadas por handlers explícitos. Novos tipos, como backup, restore, update via SteamCMD e transferências do Google Drive, serão registrados somente nas etapas que os implementarem.
