...
> Status: Em desenvolvimento. A base FastAPI/Jinja2, o layout Tailwind, métricas, health checks, controles do Palworld, logs, integração REST administrativa, editor do INI e backup/Restore local e remoto via rclone estão implementados; integrações e regras operacionais adicionais permanecem planejadas.
