...
> Status: Em desenvolvimento. A base FastAPI/Jinja2, o layout Tailwind, métricas, health checks, controles do Palworld, logs, integração REST administrativa, editor do INI, backup/Restore local e Google Drive via rclone estão implementados; integrações e regras operacionais adicionais permanecem planejadas.
