...
class RestoreJobConflictError(RuntimeError):
    """Já existe um Restore local pendente ou em execução."""
...
def enqueue_local_restore(
    ...
    *,
    ...
    if active is not None:
        raise RestoreJobConflictError("Já existe um Restore local em andamento.")
    job = Job(
    ...
    try:
        ...
        raise RestoreJobConflictError("Já existe um Restore local em andamento.") from error
    record_audit_event(
        session,
        ...
        job_id=job.id,
        target="Backup local",
        details={"backup_record_id": record.id, "source_filename": record.filename},
    )
    return job
...
class LocalRestoreJobExecutor:
    ...
    def execute(self, job_id: int) -> None:
        ...
        try:
            ...
            with session_scope(self._session_factory) as session:
                record = session.get_one(BackupRecord, request_data["backup_record_id"])
    ...
    def _load_request(self, job_id: int) -> dict[str, int | str]:
        with session_scope(self._session_factory) as session:
            ...
            if job.kind != LOCAL_RESTORE_JOB_KIND or job.status != JOB_STATUS_RUNNING:
...
def restore_job_view(job: Job) -> RestoreJobView:
    if job.kind != LOCAL_RESTORE_JOB_KIND:
        raise ValueError("job não pertence ao Restore local")
    result = job.result or {}
    source_filename = result.get("source_filename")
...
def latest_restore_job(session: Session) -> Job | None:
    return session.scalar(
        select(Job).where(Job.kind == LOCAL_RESTORE_JOB_KIND).order_by(Job.id.desc()).limit(1)
    )
